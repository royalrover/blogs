## deno
node.js之父Ryan Dahl在一个月前发起了名为deno的项目，项目的初衷是**打造一个基于v8引擎的安全的TypeScript运行时**，同时实现HTML5的基础API。所谓的安全运行时，是将TS代码运行在一个沙盒里，访问受限的文件系统、网络功能，这比较类似于web里的iframe sandbox。
